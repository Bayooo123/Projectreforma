...
export default function TodaysActivityPanel({ activities }: TodaysActivityPanelProps) {
    ...
    const getStatusStyle = (status: string) => {
        ...
        return 'bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-300 border border-border border';
    };
    ...
    if (activities.length === 0) {
        return (
            <div className="bg-white/60 dark:bg-slate-900/40 backdrop-blur-xl rounded-2xl p-10 mb-8 border border-white/20 dark:border-white/5 text-center shadow-lg">
                ...
                <p className="text-secondary dark:text-slate-400 max-w-sm mx-auto">
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
            ...
            <div className="bg-white/60 dark:bg-slate-900/40 backdrop-blur-xl rounded-2xl border border-white/20 dark:border-white/5 shadow-lg overflow-hidden flex flex-col h-full">
                <div className="p-6 border-b border-border border/50 dark:border-slate-700/50 bg-white/40 dark:bg-slate-800/40 backdrop-blur-sm">
                    <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-lg font-bold text-primary tracking-tight">Court Schedule</h3>
                ...
                <div className="p-6 flex-1">
                    {courtItems.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-center py-8 opacity-70">
                            <Scale className="w-8 h-8 text-slate-300 dark:text-tertiary mb-2" />
                            <p className="text-secondary dark:text-slate-400 text-sm">No courtroom appearances today</p>
                        </div>
                    ) : (
                        <div className="space-y-4">
                            {courtItems.map(item => (
                                <div key={item.id} className="flex items-start gap-4 p-4 rounded-xl bg-white/50 dark:bg-slate-800/50 border border-border border/50 dark:border-slate-700/50 shadow-sm hover:shadow-md hover:border-indigo-200 dark:hover:border-indigo-800 transition-all duration-300 group">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-start justify-between mb-2">
                                            <h4 className="font-bold text-primary truncate pr-2 text-base">
                                                {item.title}
                                            </h4>
                                        ...
                                        <p className="text-sm font-medium text-tertiary dark:text-slate-300 mb-3 truncate">
                                            {item.subtitle}
                                        </p>
                                        <div className="flex items-center gap-4 text-xs font-medium text-secondary dark:text-slate-400">
                                            <div className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-slate-100 dark:bg-slate-800 border border-border border">
            ...
            <div className="bg-white/60 dark:bg-slate-900/40 backdrop-blur-xl rounded-2xl border border-white/20 dark:border-white/5 shadow-lg overflow-hidden flex flex-col h-full">
                <div className="p-6 border-b border-border border/50 dark:border-slate-700/50 bg-white/40 dark:bg-slate-800/40 backdrop-blur-sm">
                    <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-lg font-bold text-primary tracking-tight">Deadlines & Filings</h3>
                ...
                <div className="p-6 flex-1">
                    {deadlineItems.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-center py-8 opacity-70">
                            ...
                            <p className="text-secondary dark:text-slate-400 text-sm">No deadlines due today</p>
                        </div>
                    ) : (
                        <div className="space-y-4">
                            {deadlineItems.map(item => (
                                <div key={item.id} className="flex items-start gap-4 p-4 rounded-xl bg-white/50 dark:bg-slate-800/50 border border-border border/50 dark:border-slate-700/50 shadow-sm hover:shadow-md hover:border-rose-200 dark:hover:border-rose-800 transition-all duration-300 group">
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-start justify-between mb-2">
                                            <h4 className="font-bold text-primary truncate pr-2 text-base">
                                                {item.title}
                                        ...
                                        <div className="flex items-center gap-2 text-xs font-medium text-secondary dark:text-slate-400">
