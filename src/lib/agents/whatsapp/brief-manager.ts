...
const WHATSAPP_TOOLS: Anthropic.Tool[] = [
    ...
    {
        name: 'send_document',
        description: 'Send an already-filed document to this WhatsApp number as a file attachment — use this whenever the user asks to be sent, shown, forwarded, or given a copy of a document that\'s already on Reforma. Only call this once you have one specific document_id in hand. If the user only named a brief (e.g. "send me the document under Osinowo v Lukefield") with nothing narrowing it to one file, do NOT call this yet — see the document disambiguation rule below.',
        input_schema: {
            type: 'object' as const,
            properties: {
...
Never guess which document when several could match — this is non-negotiable, same principle as brief lookups above:
- "Send me the document under X" / "send me that file from X" naming only a brief, with nothing else narrowing it down, is not enough to pick a document_id yourself. Call get_brief_detail for that brief first — it returns every document's id, name, and hasFile (whether there's an actual file behind it — some entries, typically older correspondence logged for the chronology, have none).
- When listing or picking candidates for a send request, only count documents with hasFile true as real candidates — one with hasFile false cannot be sent no matter what the user picks. Exactly one true candidate, or exactly one obviously matches what the user described → send it directly with send_document. Don't ask "did you mean X?" when there's only one real candidate.
- More than one true candidate and nothing narrows it to one → show them as a short numbered list by name ("1. Tenancy Agreement.pdf\\n2. Demand Letter.docx\\n3. Affidavit of Service.pdf\\nWhich one?") and wait for the reply — never pick one for them, never include a hasFile:false entry in this list, and never send more than one file without being asked for all of them. When they answer with a number or a name close to one on the list, match it against the list you just showed and immediately call send_document with that document's id — don't just confirm the pick and stop.
- If the specific document the user names or picks has hasFile false, say so plainly (it's on record but there's no file attached) rather than calling send_document and relaying whatever error comes back — and if other, sendable documents exist on the brief, offer those instead of leaving it there.
- A brief with no documents at all, or none with an actual file, is a real, reportable answer ("there are no documents filed under X yet") — not a dead end to route around.
...
- "Send/share/forward/give me [a document]" → send_document. Resolve which document first — see "Never guess which document when several could match" above.
